...
\emp
\hfill\pig{2.3in}{figs/nyst_nonsm}
...
\emp
\pig{.6in}{figs/domainGRF}
\hfill
\pig{.6in}{figs/dipole_lap}
...
\emp
\only<1-5>{\pig{.6in}{figs/domainplain}}%
\only<6->{\pig{.6in}{figs/domainyt}}
...
\quad\com{GRF $u = \Srep u^-_n - \Drep u^-$ \; $\stackrel{\text{JRs}}{\longrightarrow}$ \;
  $u_n^- = (D^T+I/2)u_n^- - Tu^-$ \;
...
\pause
    ...
    \emp
    \hfill
    \only<5->\pig{1.6in}{figs/TE-TM-formalism}
\sg
\pause
...
\begin{frame}\ft{Helmholtz --- scattering formalism}
  ...
  $u_\tbox{tot} = u_i + u$
    \emp
    \hfill
    \pig{0.8in}{figs/scattering-formalism}
...
\pause
    ...
    \emp\hfill
    \pig{1.2in}{figs/helmholtz-fundsol}
...
\pause
    ...
    \emp
    \hspace{0.3in}
    \pig{0.6in}{figs/slp_helm.pdf}\quad\com{SLP}
    \hspace{0.1in}
    \pig{0.6in}{figs/dlp_helm.pdf}\quad\com{DLP}
...
\emp
\hfill
\only<3->{\pig{1.0in}{figs/helm-transmission}}
\pause

...
eps = 1e-12;
u = kerndlap.fmm(eps,s,t,dens);

figure(3)
clf
h = pcolor(reshape(u,size(xx))); set(h,'EdgeColor','none');
clim([-5,5])
colormap(redblue)
...
% visualize the same comp at many targets 

% we can visualize by placing targets on a grid 

x1 = linspace(-2,2,300); [xx,yy] = meshgrid(x1,x1);
t = [];
t.r = [xx(:).'; yy(:).'];
...
eps = 1e-12;
ucomp = kerns.fmm(eps,chnkr,t,(chnkr.wts(:).*unbdry)) - ...
    kernd.fmm(eps,chnkr,t,(chnkr.wts(:).*ubdry));

utrue = kerns.eval(s,t)*strength;

erru = abs(ucomp-utrue);
...
figure(5)
clf
h = pcolor(xx,yy,log10(errplot)); set(h,'EdgeColor','none');
colormap("jet"  )
colorbar
hold on
plot(chnkr,'k-o')

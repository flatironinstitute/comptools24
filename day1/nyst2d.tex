...
\begin{frame}\ft{Helmholtz --- introduction and connection to Maxwell}
  ...
  \sg
    Arises from scalar wave equation $u_{tt}- \Delta u = 0$ \com{Take Fourier transform wrt $t$}
...
\sg
    ...
    Dirichlet BC in TE formalism = PEC \quad \com{perfect electric conductor; $\mathbf{E}$ $||$ to surface} 
...
\begin{frame}\ft{Helmholtz --- scattering formalism}
  ...
  $u^\tbox{tot} = u^\tbox{inc} + u$
    ...
    \quad $ (\Delta + \omega^2)u = 0 \quad \text{in } \mathbb{R}^d \backslash \bar{\Omega} \quad \text{\com{PDE}}$

    \quad $ u = -u_i \quad \text{on } \Gamma \quad \text{\com{Dirichlet BC, $u_n = -(u_i)_n$ for Neumann}}$

    \quad $ \lim_{r \to \infty} \left( \frac{\partial u}{\partial r} - iku \right) = 0 \quad\text{\com{$r := |\xx - \yy|$, Sommerfeld radiation condition for uniqueness}}$
